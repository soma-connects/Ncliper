...
export function EditorView({ projectTitle, projectId, initialClips = [] }: EditorViewProps) {
    ...
    return (
        <div className="h-[calc(100vh-140px)] flex flex-col animate-in fade-in zoom-in-95 duration-500">
            ...
            {/* Main Editor: 2-Column Layout (OpusClip Style) */}
                ...
                <div className={cn(
                    ...
                    <ClipRail
                        ...
                        onSelect={(clip) => {
                            ...
                            setActiveMatch(null); // Clear any prompt-to-clip search
                            if (window.innerWidth < 1024) setSelectedTab('player');
                        }}
                        activeClipId={selectedClip?.id}
                    />
                </div>

                {/* 2. Main Player + Details (Right - OpusClip Style) */}
                <div className={cn(
                    "flex-1 flex-col overflow-y-auto lg:flex",
